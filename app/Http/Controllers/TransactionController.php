...
class TransactionController extends Controller
{
    ...
    public function simpanTiketBaru(Request $request){
        ...
        $data->id_teknisi       = '';
        $data->no_internet      = $request->nointernet;
        $data->alamat           = $request->alamat;
        $data->relasi           = $request->relasi;
    ...
    public function detailticket($id){
        ...
        $user       = Customer::where('email','=', $data->reported)->first();
    ...
    public function updateStatus(Request $request){
        ...
        if($request->aksi == 'tolaktiket')
        {
            ...
            \Mail::to($tiket->customer->email)->send(new \App\Mail\TicketDitolak($data));
            //update
            ...
            return redirect()->route('list.ticket')->with('message','Request '.$request->id.' Berhasil Ditolak!');
        }
        ...
        if($request->aksi == 'update_progress'){
            ...
            $treking = [
                'id_ticket' => $request->idTiket,
                'status'    => ($request->progress < 100) ? 'Request sedang dikerjakan' : 'Request solved',
                'deskripsi' => $request->komentar,
                'id_user'   => Auth::user()->username,
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s')
            ...
            return redirect()->route('detail.ticket.teknisi',$request->idTiket)->with('message','Request Berhasil Diupdate!');
        }
    }
